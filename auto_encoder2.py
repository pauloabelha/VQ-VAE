...
class CVAE_YCB(AbstractAutoEncoder):
    def __init__(self, d, kl_coef=0.1, **kwargs):
        super(CVAE_YCB, self).__init__()
...
class VQ_CVAE(nn.Module):
    ...
    def loss_function_adversarial(self, x, recon_x, z_e, emb, argmin):
        ...
        self.mse_hand_obj_adv = F.mse_loss(recon_hand, recon_obj) / (640*480)
        adv_loss = (-self.mse_hand_adv) + (-self.mse_obj_adv) + (-self.mse_hand_obj_adv)
        self.recon_loss = 3.0*self.mse + adv_loss + 1.0
